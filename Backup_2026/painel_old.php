<?php
// ==========================================================
// ARQUIVO: painel.php (ATUALIZADO 25/01/2026 - Menu Tutoriais)
// ==========================================================
...
// Identifica o usuário (Garantido pelo validator)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_acao'])) {
    ...
    try {
        ...
        elseif ($acao === 'mudar_data') {
            $nova_data = $_POST['nova_data']; // Esperado YYYY-MM-DD
            if (empty($nova_data)) throw new Exception("Data inválida.");
            
            // Mantém hora atual ou fixa meio-dia para evitar problemas de fuso
...
<body class="text-slate-200 font-sans antialiased selection:bg-brand-accent selection:text-brand-dark">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    <!-- NOVO: Link Backups -->
                    <a href="meus_backups.php" class="text-sm font-medium text-brand-accent hover:text-white transition-colors flex items-center gap-2">
                    ...
                    <?php endif; ?>
                     ...
                     <!-- CANAL DE SUPORTE -->
                    <a href="https://api.whatsapp.com/send?phone=5531971875928&text=Ol%C3%A1%2C%20preciso%20reportar%20um%20problema%20ou%20tirar%20d%C3%BAvida%20sobre%20o%20SGT." target="_blank" class="px-3 py-2 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white text-xs font-bold rounded-lg border border-white/10 transition-colors flex items-center gap-2" title="Reportar Problema / Suporte">
                        <i class="ph ph-life-buoy"></i> <span class="hidden lg:inline">Suporte</span>
                    </a>

                    <!-- TUTORIAIS YOUTUBE (DROPDOWN) -->
                    <div class="relative group">
                        <button class="px-3 py-2 bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-300 hover:text-white text-xs font-bold rounded-lg border border-emerald-600/30 transition-colors flex items-center gap-2" title="Tutoriais em Vídeo">
                            <i class="ph ph-graduation-cap"></i> <span class="hidden lg:inline">Tutoriais</span>
                            <i class="ph ph-caret-down text-[10px] opacity-70"></i>
                        </button>
                        <!-- Dropdown Menu -->
                        <div class="absolute right-0 mt-2 w-56 glass-panel rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50">
                            <div class="py-2">
                                <div class="px-4 py-1 text-[10px] text-slate-500 uppercase font-bold tracking-wider">Vídeos Tutoriais</div>
                                <!-- LISTA DE TUTORIAIS - ADICIONE NOVOS AQUI -->
                                <a href="https://youtu.be/1FFl1Hk2uFY?si=Wra6C0LvHqo7PlJ-" target="_blank" class="block px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white flex items-center gap-2">
                                    <i class="ph ph-play-circle text-emerald-400"></i> Gerar Proposta
                                </a>
                                <!-- ADICIONE MAIS LINKS AQUI -->
                                <!-- <a href="URL_DO_VIDEO" target="_blank" class="block px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white flex items-center gap-2">
                                    <i class="ph ph-play-circle text-emerald-400"></i> Nome do Tutorial
                                </a> -->
                            </div>
                        </div>
                    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
            ...
            <div class="glass-card p-5 rounded-2xl flex items-center gap-4 group">
                ...
                <div>
                    <p class="text-xs text-slate-400 uppercase font-bold tracking-wider">Elaboradas (Mês)</p>
            ...
            <div class="glass-card p-5 rounded-2xl flex items-center gap-4 group">
                ...
                <div>
                    <p class="text-xs text-slate-400 uppercase font-bold tracking-wider">Enviadas (Mês)</p>
            ...
            <div class="glass-card p-5 rounded-2xl flex items-center gap-4 group border-green-500/20 bg-green-500/5">
                ...
                <div>
                    <p class="text-xs text-green-400/80 uppercase font-bold tracking-wider">Aprovadas (Mês)</p>
            ...
            <div class="glass-card p-5 rounded-2xl flex items-center gap-4 group border-red-500/20">
                ...
                <div>
                    <p class="text-xs text-slate-400 uppercase font-bold tracking-wider">Canceladas (Mês)</p>
                ...
                <table class="w-full text-left border-collapse" id="tabelaPrincipal">
                    ...
                    <tbody class="divide-y divide-white/5 text-sm">
                        ...
                        <tr class="hover:bg-white/5 transition-colors group">
                            <td class="px-6 py-4 text-slate-400 font-mono relative group/date">
                                <span onclick="abrirEditorData(<?= $id ?>, '<?= date('Y-m-d', strtotime($row['data_criacao'])) ?>')" class="cursor-pointer hover:text-white transition-colors flex items-center gap-2">
                                    <?= date('d/m/Y', strtotime($row['data_criacao'])); ?>
                                    <i class="ph ph-pencil-simple text-brand-accent opacity-0 group-hover/date:opacity-100 transition-opacity" title="Alterar Data"></i>
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="font-bold text-white mb-2">R$ <?= number_format($row['valor_final_proposta'], 2, ',', '.'); ?></div>
                                
                                <div class="flex items-center justify-end gap-3 actions-row">
                                    <a href="editar_proposta.php?id=<?= $id ?>" class="inline-flex items-center gap-1 text-xs text-yellow-500 hover:text-white transition-colors" title="Editar Proposta">
                                        <i class="ph ph-pencil-simple text-lg"></i>
                                    </a>
                                    
                                    <a href="relatorio_proposta.php?id=<?= $id ?>" class="inline-flex items-center gap-1 text-xs text-brand-accent hover:text-white transition-colors" title="Ver Relatório">
                                        <i class="ph ph-file-text text-lg"></i>
                                    </a>

                                    <?php
                                        // --- LÓGICA MULTI-ESTRATÉGIA DE ARQUIVO (RECURSIVA) ---
                                        $anoProp = date('Y', strtotime($row['data_criacao']));
                                        $numeroProposta = $row['numero_proposta'];
                                        
                                        // Extrai SEQUENCIA - Lida com revisões (Ex: GEOMETRPOLE-2026-007-Rv01)
                                        ...
                                        if (preg_match('/^Rv\d+$/i', end($parts))) {
                                            $sufixoRevisao = '-' . array_pop($parts); // Remove e guarda o sufixo (Ex: -Rv01)
                                            $rawSeq = end($parts); // Agora pega o número real
                                        } else {
                                            $rawSeq = end($parts);
                                        }
                                        
                                        $numPad = str_pad(preg_replace('/\D/', '', $rawSeq), 3, '0', STR_PAD_LEFT);
                                        $prefix = reset($parts); // Primeiro pedaço (Ex: GEOMETRPOLE)

                                        $bases = [];
                                        if (!empty($row['nome_cliente'])) $bases[] = $row['nome_cliente'];
                                        if (!empty($row['nome_cliente_salvo'])) $bases[] = $row['nome_cliente_salvo'];
                                        if (!empty($row['nome_empresa_proponente'])) $bases[] = $row['nome_empresa_proponente'];
                                        
                                        $caminhoFinal = '';
                                        $arquivoExiste = false;
                                        
                                        // Função auxiliar para busca recursiva
                                        $buscaRecursiva = function($dir, $pattern) use (&$buscaRecursiva) {
                                            $resultados = [];
                                            ...
                                            foreach ($subdirs as $subdir) {
                                                $subResultados = $buscaRecursiva($subdir, $pattern);
                                        ...
                                        // 1. Tentativa Exata (Bases Conhecidas) - COM busca recursiva
                                        foreach($bases as $base) {
                                            $nomeLimpo = preg_replace('/[^A-Za-z0-9]/', '', iconv('UTF-8', 'ASCII//TRANSLIT', $base));
                                            
                                            // Tenta COM e SEM sufixo de revisão
                                            $patterns = [
                                                $nomeLimpo . '-' . $anoProp . '-' . $numPad . $sufixoRevisao . '.docx',
                                                $nomeLimpo . '-' . $anoProp . '-' . $numPad . '.docx'
                                            ];
                                            
                                            foreach ($patterns as $nomeArquivo) {
                                                // Primeiro tenta na raiz
                                                if (file_exists($dirBase . '/' . $nomeArquivo)) {
                                                    $caminhoFinal = 'propostas_emitidas/' . $nomeArquivo;
                                                    $arquivoExiste = true;
                                                    break 2; 
                                                }
                                                // Depois busca recursivamente
                                                $encontrados = $buscaRecursiva($dirBase, $nomeArquivo);
                                                if ($encontrados) {
                                                    $caminhoFinal = str_replace(__DIR__ . '/', '', $encontrados[0]);
                                                    $arquivoExiste = true;
                                                    break 2;
                                                }
                                            }
                                        ...
                                        // 2. Fallback: Busca via Prefixo (Recursiva + Revisões)
                                        if (!$arquivoExiste && !empty($prefix)) {
                                            $prefixClean = preg_replace('/[^A-Za-z0-9]/', '', iconv('UTF-8', 'ASCII//TRANSLIT', $prefix));
                                            
                                            // Padrões de busca (com e sem sufixo de revisão)
                                            $searchPatterns = [
                                                '*-' . $anoProp . '-' . $numPad . $sufixoRevisao . '.docx',
                                                '*-' . $anoProp . '-' . $numPad . '.docx',
                                                '*-' . $numeroProposta . '.docx'
                                            ];
                                            
                                            foreach ($searchPatterns as $pattern) {
                                                $candidatos = $buscaRecursiva($dirBase, $pattern);
                                                
                                                if ($candidatos) {
                                                    foreach($candidatos as $arqPath) {
                                                        $nomeArq = basename($arqPath);
                                                        // Verifica se começa com o prefixo (Case Insensitive)
                                                        if (stripos($nomeArq, $prefixClean) === 0) {
                                                            $caminhoFinal = str_replace(__DIR__ . '/', '', $arqPath);
                                                            $arquivoExiste = true;
                                                            break 2;
                                                        }
                                                    }
                                        ...
                                        if (!$arquivoExiste) {
                                            // Remove sufixo de revisão para busca mais ampla
                                            ...
                                            foreach ($fallbackPatterns as $pattern) {
                                                ...
                                                if ($candidatos) {
                                                    ...
                                                    $caminhoFinal = str_replace(__DIR__ . '/', '', $candidatos[0]);
                                                    $arquivoExiste = true;
                                                    break;
                                                }
                                            }
                                    ...
                                    <?php if($arquivoExiste): ?>
                                        <a href="enviar_email.php?id=<?= $id ?>" class="inline-flex items-center gap-1 text-xs text-blue-400 hover:text-white transition-colors" title="Enviar por E-mail">
                                            <i class="ph ph-paper-plane-tilt text-lg"></i>
                                        </a>

                                        <a href="<?= $caminhoFinal ?>" download class="inline-flex items-center gap-1 text-xs text-green-400 hover:text-white transition-colors" title="Baixar DOCX">
                                            <i class="ph ph-download-simple text-lg"></i>
                                        </a>
                                    <?php else: ?>
                                        <span class="inline-flex items-center gap-1 text-xs text-slate-700 cursor-not-allowed opacity-30" title="Arquivo não encontrado.">
                                            <i class="ph ph-paper-plane-tilt text-lg"></i>
                                        </span>
                                        <span class="inline-flex items-center gap-1 text-xs text-slate-700 cursor-not-allowed opacity-30" title="Arquivo não encontrado.">
                                            <i class="ph ph-download-simple text-lg"></i>
    ...
    <script>
        ...
        function abrirEditorData(id, dataAtual) {
            // Usa prompt simples para trocar a data (Rápido e eficaz)
            const novaData = prompt("Corrigir Data da Proposta (AAAA-MM-DD):", dataAtual);
            
            if (novaData && novaData !== dataAtual) {
                // Validação simples de formato YYYY-MM-DD
                if(!/^\d{4}-\d{2}-\d{2}$/.test(novaData)) {
                    alert("Formato inválido! Use AAAA-MM-DD (Ex: 2025-12-28)");
                    return;
                }
                ...
                .then(res => {
                    if (res.sucesso) {
                        // Recarrega para reordenar
                        window.location.reload(); 
                    } else {
                        alert('Erro ao alterar data: ' + res.msg);
                    }
                })
                .catch(e => {
                    console.error(e);
                    alert('Erro de conexão ao salvar data.');
                })
                .finally(() => document.body.style.cursor = 'default');
            }
        }
