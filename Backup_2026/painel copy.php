...
<body>
    ...
    <?php if($is_demo && isset($_SESSION['validade_demo'])): 
        ...
        $corAlerta = ($diasRestantes <= 1) ? 'danger' : 'warning';
    ...
    <div class="container-fluid px-4 mb-3">
        <div class="alert alert-<?= $corAlerta ?> d-flex justify-content-between align-items-center shadow-sm">
            <div>
                <strong><?= $textoAlerta ?></strong>
                <span class="d-none d-md-inline ms-2">Contrate agora para manter seu acesso e salvar seus dados.</span>
            </div>
            <a href="contratar.php" class="btn btn-dark btn-sm fw-bold">CONTRATAR AGORA</a>
        </div>
    </div>
    <?php endif; ?>
    ...
    <!-- MODAL NOVIDADES (What's New) -->
    <div class="modal fade" id="modalNovidades" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title fw-bold"><i class="bi bi-stars me-2"></i> Novidades da Versão <span id="novidadeVersao"></span></h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-4">
                    <h4 id="novidadeTitulo" class="fw-bold text-dark mb-3"></h4>
                    <div id="novidadeDescricao" class="text-secondary"></div>
                </div>
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-primary px-4 fw-bold" data-bs-dismiss="modal">Entendi, vamos lá!</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // VERIFICAÇÃO DE NOVIDADES (Ao carregar)
        document.addEventListener('DOMContentLoaded', function() {
            // Verifica se tem novidade não lida
            fetch('api_check_novidades.php')
                ...
                .then(res => {
                    if (res.tem_novidade) {
                        ...
                        var modal = new bootstrap.Modal(document.getElementById('modalNovidades'));
                        modal.show();

                        // Marca como lida ao fechar (ou imediatamente ao abrir, para não irritar)
