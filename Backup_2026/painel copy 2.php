...
try {
    ...
    // 2.1 Lógica de Data Dinâmica (Sincronizada com Gráficos)
    ...
    $dataRef = $resMax['max_data'] ? new DateTime($resMax['max_data']) : new DateTime();
    $dataRef->modify('last day of this month'); $dataRef->setTime(23,59,59);
    $strFim = $dataRef->format('Y-m-d H:i:s');
    
    $dataIni = clone $dataRef;
    $dataIni->modify('-5 months'); $dataIni->modify('first day of this month'); $dataIni->setTime(0,0,0);
    $strInicio = $dataIni->format('Y-m-d H:i:s');
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-lg-4">
                <div class="d-flex flex-column gap-4 h-100">
                    <!-- Gráfico Pizza -->
                    <div class="chart-box flex-grow-1" style="min-height: 300px;">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div class="card-header bg-white fw-bold py-3 border-bottom">Resumo Financeiro</div>
                        <div class="card-body p-0">
                            <table class="table table-sm mb-0" style="font-size: 0.9rem;">
                                <tbody>
                                    <tr>
                                        <td class="ps-3"><i class="bi bi-circle-fill text-success me-2" style="font-size: 8px;"></i>Aceitas</td>
                                        <td class="text-end pe-3 fw-bold text-success" id="resumo-aprovada">R$ 0,00</td>
                                    </tr>
                                    <tr>
                                        <td class="ps-3"><i class="bi bi-circle-fill text-primary me-2" style="font-size: 8px;"></i>Enviadas</td>
                                        <td class="text-end pe-3 fw-bold text-primary" id="resumo-enviada">R$ 0,00</td>
                                    </tr>
                                    <tr>
                                        <td class="ps-3"><i class="bi bi-circle-fill text-warning me-2" style="font-size: 8px;"></i>Elaboradas</td>
                                        <td class="text-end pe-3 fw-bold text-warning" id="resumo-elaborada">R$ 0,00</td>
                                    </tr>
                                    <tr>
                                        <td class="ps-3"><i class="bi bi-circle-fill text-danger me-2" style="font-size: 8px;"></i>Canceladas</td>
                                        <td class="text-end pe-3 fw-bold text-danger" id="resumo-cancelada">R$ 0,00</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
        ...
        function carregarDadosDashboard() {
            fetch('api_graficos.php').then(r => r.json()).then(data => {
                ...
                if(data.resumo_financeiro) {
                    document.getElementById('resumo-aprovada').textContent = fmt.format(data.resumo_financeiro.Aprovada || 0);
                    document.getElementById('resumo-enviada').textContent = fmt.format(data.resumo_financeiro.Enviada || 0);
                    document.getElementById('resumo-elaborada').textContent = fmt.format(data.resumo_financeiro['Em elaboração'] || 0);
                    document.getElementById('resumo-cancelada').textContent = fmt.format(data.resumo_financeiro.Cancelada || 0);
                }

                // Limites vindos do Banco de Dados (Plano Cartesiano Real)
                let dbMin = data.eixo_y.min || 0;
                let dbMax = data.eixo_y.max || 1000;
                
                // Define Folga (Padding) de 10% sobre o intervalo real
                let range = dbMax - dbMin;
                if (range === 0) range = dbMax * 0.1 || 100;
                
                let padding = range * 0.10;
                
                // Define os limites exatos do gráfico
                let finalMin = Math.max(0, dbMin - padding);
                let finalMax = dbMax + padding;

                // Função para tratar zeros: substitui pelo MENOR valor da lista
                const treatData = (arr) => arr.map(v => v === 0 ? dbMin : v);
                ...
                window.chartLinhaInstance = new Chart(ctxLinha, { 
                    ...
                    data: { 
                        ...
                        datasets: [ 
                            { label: 'Aceitas', data: treatData(data.grafico_linha.series['Aprovada']), borderColor: '#198754', backgroundColor: '#198754', tension: 0.3, borderWidth: 3, pointRadius: 4, fill: false }, 
                            { label: 'Enviadas', data: treatData(data.grafico_linha.series['Enviada']), borderColor: '#0d6efd', backgroundColor: '#0d6efd', tension: 0.3, borderWidth: 3, pointRadius: 4, fill: false }, 
                            { label: 'Elaboradas', data: treatData(data.grafico_linha.series['Em elaboração']), borderColor: '#ffc107', backgroundColor: '#ffc107', tension: 0.3, borderWidth: 2, borderDash: [5, 5], fill: false },
                            { label: 'Canceladas', data: treatData(data.grafico_linha.series['Cancelada']), borderColor: '#dc3545', backgroundColor: '#dc3545', tension: 0.3, borderWidth: 2, pointRadius: 3, fill: false }
                        ] 
                    }, 
                    options: { 
                        responsive: true, 
