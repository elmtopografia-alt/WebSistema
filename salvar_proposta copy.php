...
// Função: Versão Final Blindada. Salva no banco e gera o Word com tratamento de erros.

// Desativa exibição de erros na tela para não quebrar o redirecionamento (header)
...
function limparStr($string) {
    // Lista Manual de Substituição (Garante remoção de acentos)
    $string = str_replace(
...
function gerarNumero($conn, $nomeEmpresa) {
    ...
    // Busca o último número para base
    $sql = "SELECT numero_proposta FROM Propostas WHERE numero_proposta LIKE CONCAT(?, '-', ?, '-%') ORDER BY id_proposta DESC LIMIT 1";
    ...
    // LOOP DE SEGURANÇA (Anti-Duplicidade)
    do {
        ...
        if ($check->num_rows > 0) {
            $prox_seq++; // Se existe, tenta o próximo
        } else {
            break; // Se não existe, usa este
        }
    } while (true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!$cliente_info || !$emp) throw new Exception("Dados incompletos (Cliente ou Empresa). Verifique os cadastros.");
        ...
        // GERA O NÚMERO (COM LOOP ANTI-DUPLICIDADE)
        ...
        $stmt->bind_param($types,
            $num_proposta, $id_cliente, $id_criador, $is_demo_int,
            $cliente_info['nome_cliente'], $cliente_info['email'], $cliente_info['telefone'], $cliente_info['celular'], $cliente_info['whatsapp'],
        ...
        // =========================================================
        // SELEÇÃO INTELIGENTE DO MODELO DOCX
        // =========================================================
        
        $nomeServicoLimpo = limparStr($nome_servico);
        $arquivoTentativa = 'ModeloProposta' . $nomeServicoLimpo . '.docx';
        
        // Verifica se existe o específico
        if (file_exists($pastaBase . $arquivoTentativa)) {
            $arquivoModelo = $arquivoTentativa;
        } else {
            $arquivoModelo = 'ModeloPropostaPadrao.docx';
        }

        // Se nem o Padrão existir, aí sim dá erro
        if (!file_exists($pastaBase . $arquivoModelo)) {
            throw new Exception("Modelo não encontrado! O sistema buscou: $arquivoTentativa e ModeloPropostaPadrao.docx.");
        }
