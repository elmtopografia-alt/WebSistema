...
class PropostaRepository 
{
    ...
    {
        // Enriquece dados flat para modelos (Drone, GPS, etc)
...
public function getAllLookupData($idUsuario)
{
    ...
    // 2.1 Adição Especial: Classificações Internas (tipos_servico) para o Wizard
    $data['arrays_js']['tipos_servico'] = [];
    $resTs = $this->conn->query("SELECT id, nome, cor, icone FROM tipos_servico ORDER BY nome ASC");
    if ($resTs) {
    ...
    {
        // Normalização: Se veio o formato Flat (Wizard Legado), agrupamos em arrays estruturados
        $salarios = $dados['salarios'] ?? [];
        if (empty($salarios) && !empty($dados['salario_id_funcao'])) {
            foreach ($dados['salario_id_funcao'] as $i => $idFuncao) {
                if (!$idFuncao) continue;
                $salarios[] = [
                    'funcao' => $idFuncao,
                    'quantidade' => $dados['salario_qtd'][$i] ?? 1,
                    'valor' => $dados['salario_valor'][$i] ?? 0,
                    'encargos' => $dados['encargos'][$i] ?? 67,
                    'dias' => $dados['salario_dias'][$i] ?? 1
                ];
            }
        }

        $estadias = $dados['estadias'] ?? [];
        if (empty($estadias) && !empty($dados['estadia_id'])) {
            foreach ($dados['estadia_id'] as $i => $idEstadia) {
                if (!$idEstadia) continue;
                $estadias[] = [
                    'tipo' => $idEstadia,
                    'quantidade' => $dados['estadia_qtd'][$i] ?? 1,
                    'valor' => $dados['estadia_valor'][$i] ?? 0,
                    'noites' => $dados['estadia_dias'][$i] ?? 1
                ];
            }
        }

        $consumos = $dados['consumos'] ?? [];
        if (empty($consumos) && !empty($dados['consumo_id'])) {
            foreach ($dados['consumo_id'] as $i => $idConsumo) {
                if (!$idConsumo) continue;
                $consumos[] = [
                    'tipo' => $idConsumo,
                    'quantidade' => $dados['consumo_qtd'][$i] ?? 1,
                    'kml' => $dados['consumo_kml'][$i] ?? 1,
        ...
        if (empty($locacoes) && !empty($dados['locacao_id'])) {
            foreach ($dados['locacao_id'] as $i => $idLocacao) {
                if (!$idLocacao) continue;
                $locacoes[] = [
                    'tipo' => $idLocacao,
                    'marca' => $dados['locacao_id_marca'][$i] ?? null,
                    'quantidade' => $dados['locacao_qtd'][$i] ?? 1,
                    'valor' => $dados['locacao_valor'][$i] ?? 0,
                    'dias' => $dados['locacao_dias'][$i] ?? 1
                ];
            }
        }

        $admin = $dados['admin'] ?? [];
        if (empty($admin) && !empty($dados['admin_id'])) {
            foreach ($dados['admin_id'] as $i => $idAdmin) {
                if (!$idAdmin) continue;
                $admin[] = [
                    'tipo' => $idAdmin,
                    'quantidade' => $dados['admin_qtd'][$i] ?? 1,
        ...
        // 1. Salários
        if (!empty($salarios) && is_array($salarios)) {
        ...
        // 2. Estadias
        if (!empty($estadias) && is_array($estadias)) {
        ...
        // 4. Locações
        if (!empty($locacoes) && is_array($locacoes)) {
    ...
    /**
     * =========================================================================
     * MÉTODOS UPGRADE v2.0 (EDICAO COMPLETA)
     * =========================================================================
     */

    public function salvarCompleto(array $dados, int $idUsuario): int 
    {
        $this->conn->begin_transaction();
        try {
            $this->preencherEquipamentosFlat($dados);
            ...
            $this->conn->rollback();
            error_log("ERRO salvarCompleto: " . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function atualizarCompleto(int $idProposta, array $dados, int $idUsuario): bool 
    {
        $this->conn->begin_transaction();
        try {
            $this->preencherEquipamentosFlat($dados);
            ...
            $this->updateProposta($idProposta, $dados, $totais);
            
            // 2. Atualiza dados planilha v2.0
            $planilha = $this->extrairDadosPlanilha($dados);
            $this->atualizarDadosPlanilhaV2($idProposta, $planilha);
            
            // 3. Sincroniza Conteúdo Personalizado (Editor)
            $this->salvarConteudoPersonalizado($idProposta, $dados);

            $this->conn->commit();
            return true;
        } catch (Throwable $e) {
            $this->conn->rollback();
            error_log("ERRO atualizarCompleto: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Mapeia os dados do $_POST para as colunas da migration v2.0
     */
    private function extrairDadosPlanilha(array $dados): array 
    {
        $planilha = [];
        ...
        for ($i = 0; $i < 5; $i++) {
            $idx = $i + 1;
            $row = $items[$i] ?? null;
            $planilha["adm_{$idx}_id"]       = isset($row['tipo']) ? intval($row['tipo']) : null;
        ...
        for ($i = 0; $i < 10; $i++) {
            $idx = $i + 1;
            $row = $items[$i] ?? null;
            $planilha["fun_{$idx}_id"]       = isset($row['funcao']) ? intval($row['funcao']) : null;
            $planilha["fun_{$idx}_qtd"]      = isset($row['quantidade']) ? floatval($row['quantidade']) : 0;
            $planilha["fun_{$idx}_dias"]     = isset($row['dias']) ? floatval($row['dias']) : 0;
        ...
        for ($i = 0; $i < 5; $i++) {
            $idx = $i + 1;
            $row = $items[$i] ?? null;
            $planilha["est_{$idx}_id"]       = isset($row['tipo']) ? intval($row['tipo']) : null;
        ...
        for ($i = 0; $i < 20; $i++) {
            $idx = $i + 1;
            $row = $items[$i] ?? null;
            $planilha["con_{$idx}_id"]       = isset($row['tipo']) ? intval($row['tipo']) : null;
            $planilha["con_{$idx}_qtd"]      = isset($row['quantidade']) ? floatval($row['quantidade']) : 0;
            $planilha["con_{$idx}_kml"]      = isset($row['kml']) ? floatval($row['kml']) : 10;
            $planilha["con_{$idx}_litro"]    = isset($row['valor_litro']) ? floatval($row['valor_litro']) : 0;
        ...
        for ($i = 0; $i < 10; $i++) {
            $idx = $i + 1;
            $row = $items[$i] ?? null;
            $planilha["loc_{$idx}_id_tipo"]  = isset($row['tipo']) ? intval($row['tipo']) : null;
            $planilha["loc_{$idx}_id_marca"] = isset($row['marca']) ? intval($row['marca']) : null;
            $planilha["loc_{$idx}_qtd"]      = isset($row['quantidade']) ? floatval($row['quantidade']) : 0;
    ...
    private function atualizarDadosPlanilhaV2(int $id, array $planilha): void 
    {
        ...
        $tmp = [];
        foreach($bindParams as $i => $v) $tmp[$i] = &$bindParams[$i];
        call_user_func_array([$stmt, 'bind_param'], $tmp);
        $stmt->execute();
    }
    ...
    private function preencherEquipamentosFlat(array &$dados) {
        ...
        $dados['modelo_estacao_total'] = "Não se aplica";
        $dados['modelo_veiculo'] = "Não se aplica";

        $locacoes = $dados['locacoes'] ?? [];
        
        // Normalização: Se veio no formato flat (Wizard), converte para estruturado
        if (empty($locacoes) && !empty($dados['locacao_id'])) {
            foreach ($dados['locacao_id'] as $i => $idLoc) {
                if (!$idLoc) continue;
                $locacoes[] = [
                    'tipo' => $idLoc,
                    'marca' => $dados['locacao_id_marca'][$i] ?? null
        ...
        foreach ($locacoes as $l) {
            ...
            // Busca nomes no banco
            $sql = "SELECT tl.nome as tipo_nome, m.nome_marca as marca_nome 
                    FROM Tipo_Locacao tl 
                    LEFT JOIN Marcas m ON m.id_marca = ?
                    WHERE tl.id_locacao = ?";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param('ii', $idMarca, $idTipo);
            $stmt->execute();
            $res = $stmt->get_result();
            $item = $res->fetch_assoc();

            if ($item) {
                $tipo = mb_strtolower($item['tipo_nome']);
                $marcaStr = $item['marca_nome'] ?? 'Não informada';
                
                // Mapeamento por palavra-chave no nome do tipo
                if (strpos($tipo, 'drone') !== false) {
                    $dados['modelo_drone'] = $marcaStr;
                } elseif (strpos($tipo, 'gps') !== false || strpos($tipo, 'gnss') !== false) {
                    $dados['modelo_gps'] = $marcaStr;
                } elseif (strpos($tipo, 'estação') !== false || strpos($tipo, 'estacao') !== false) {
                    $dados['modelo_estacao_total'] = $marcaStr;
